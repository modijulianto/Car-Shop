...
class _ListMerkState extends State<ListMerk> {
  Future<List> getData() async {
    final response =
        await http.get("http://192.168.43.58/apicarshop/Carshop/merk");
            ...
            child: new Hero(
              ...
              child: new Material(
                child: new InkWell(
                  ...
                  child: new ListMobil(merk: merk, logo: logo, idmerk: idmerk.toString(),),
                )
              )
            ),
          ),
    ...
    return new Scaffold(
      ...
      body: new FutureBuilder<List>(
        ...
        builder: (context, snapshot){
          ...
          return snapshot.hasData
          ? new ListCar(list: snapshot.data, merk: widget.merk, logo: widget.logo, idmerk: widget.idmerk.toString(),)
          : new Center(child: new CircularProgressIndicator(),);
        }
      ),
    );
...
class _ListCarState extends State<ListCar> {
  ...
  Widget build(BuildContext context) {
    return new  ListView.builder(
      ...
      itemBuilder: (context, i){
        if(widget.list[i]['id_merk'] == widget.idmerk){
          return new Container(
